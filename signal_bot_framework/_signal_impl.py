...
from .args import SendMessageArgs, ListContactArgs, GetUserStatusArgs
...
class SignalBotImpl(SignalBot, Personality, JsonRpcHandler):
    ...
    async def get_group_info(self, groupId: GroupId) -> Future[Response]:
        return asyncio.ensure_future(
            Response.from_future_frame(await self.__json_rpc('listGroups', groupId=groupId)))
    
    async def list_contacts(self, args: ListContactArgs | None = None) -> Future[Response]:
        kwargs = {}
        if args is not None:
            kwargs.update(args.to_args())
        return asyncio.ensure_future(Response.from_future_frame(await self.__json_rpc('listContacts', **kwargs)))

    async def get_user_status(self, args: GetUserStatusArgs) -> Future[Response]:
        kwargs = {}
        kwargs.update(args.to_args())
        return asyncio.ensure_future(Response.from_future_frame(await self.__json_rpc('getUserStatus', userId)))
